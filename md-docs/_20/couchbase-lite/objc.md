...
The automatic upgrade functionality does not copy any conflicting revisions to the new database. The motivation for this functionality is related to the way conflicts are being handled in Couchbase Lite 2.0 (see [Handling Conflicts](objc.html#handling-conflicts)).
...
If the property doesn't exist in the document it will return the default value for that getter method (0 for `getInt`, 0.0 for `getFloat` etc.). To check whether a given property exists in the document, you should use the [`[Document contains:key]`](http://docs.couchbase.com/mobile/2.0/couchbase-lite-objc/db022/Classes/CBLDocument.html) method.
